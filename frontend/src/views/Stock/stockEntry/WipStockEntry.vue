  ...
  <div class="wip-stock-page">
    ...
    <el-card class="form-card" shadow="never">
      <div class="form-header">
        ...
        <div class="header-content">
          ...
          <div class="title-section">
            <h2 class="form-title">仕掛品在庫 入出庫登録</h2>
            <p class="form-subtitle">工程間の仕掛品在庫を管理</p>
          </div>
        </div>
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-width="90px"
        class="transaction-form align-form vertical-form" label-position="left">
        ...
        <div class="form-section">
          ...
          <!-- 工程CD -->
          <el-form-item label="工程" prop="process_cd" class="elegant-form-item">
          ...
          <el-form-item label="保管場所" prop="location_cd" class="elegant-form-item">
            <div class="location-container">
              <el-radio-group v-model="form.location_cd" class="elegant-location-group">
                <el-radio-button v-for="loc in locationOptions" :key="loc.cd" :value="loc.cd" class="elegant-location-btn">
                  {{ loc.name }}
                </el-radio-button>
              </el-radio-group>
            </div>
    ...
    <!-- 本日登録した仕掛品一覧 -->
    <el-card v-if="todayLoggedWips.length" class="result-card" shadow="never">
      ...
      <div class="table-container">
        <el-table :data="todayLoggedWips" stripe class="elegant-table">
...
import { getProductOptions, getProcessOptions } from '@/api/options';
...
const createInitialForm = (): WipStockForm => ({
  target_cd: '',
  process_cd: '',
  location_cd: '工程中間在庫',
  transaction_type: '',
...
const form = ref<WipStockForm>(createInitialForm());
...
// 当日登録した仕掛品リスト
const todayLoggedWips = ref<LoggedWip[]>([]);
...
const submit = async () => {
  ...
  try {
    ...
    if (form.value.transaction_time.startsWith(todayYMD)) {
      const prod = productOptions.value.find(p => p.cd === form.value.target_cd);
      const proc = processOptions.value.find(p => p.cd === form.value.process_cd);
      if (
        prod &&
        proc &&
        !todayLoggedWips.value.some(
          w =>
            w.cd === prod.cd &&
            w.process_name === proc.name &&
            w.transaction_type === form.value.transaction_type &&
            w.quantity === form.value.quantity
        )
      ) {
        todayLoggedWips.value.push({
          cd: prod.cd,
          name: prod.name,
...
.elegant-location-group {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
}

.elegant-location-btn :deep(.el-radio-button__inner) {
  background: rgba(255, 255, 255, 0.8);
...
.elegant-location-btn :deep(.el-radio-button__inner):hover {
...
.elegant-location-btn :deep(.el-radio-button__orig-radio:checked + .el-radio-button__inner) {
