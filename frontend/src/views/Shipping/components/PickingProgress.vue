...
const fetchProgressData = async () => {
  ...
  try {
    ...
    if (response && typeof response === 'object') {
      palletList.value = response.palletList || []
      progressStats.value = response.progressStats || generateTestData() // 使用测试数据作为备用
      todayOverview.value = response.todayOverview || {
        total_today: 45,
        pending_today: 12,
        completed_today: 33,
...
const initChart = () => {
  ...
  if (containerRect.width === 0 || containerRect.height === 0) {
    if (initRetryCount < MAX_INIT_RETRIES) {
      initRetryCount++
      console.warn(
        `Container size is 0, retrying ${initRetryCount}/${MAX_INIT_RETRIES} in 200ms...`,
      )
      setTimeout(() => {
        initChart()
      }, 200)
...
const updateChart = () => {
  ...
  try {
    ...
    const rates = progressStats.value.map((stat, index) => {
