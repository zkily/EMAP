      ...
      >
        ...
        <div class="print-body">
          <div class="column" v-for="colIndex in 2" :key="colIndex">
            <div
              v-for="destGroup in getColumnData(pageData, colIndex - 1)"
              :key="`${destGroup.destination_name}-${pageIndex}-${colIndex}`"
...
function getDestinationSize(dest) {
  ...
  // 预估每行可能需要的额外空间（考虑换行）
  const estimatedExtraSpace = Math.ceil(itemCount * 0.3) // 预留30%的额外空间用于换行
...
function allocateNounyusenToColumns(nounyusenGroup, columns, columnSizes) {
  // 调整后的容量限制 - 考虑换行后的行高增加
  const maxItemsPerColumn = 32 // 减少到32个商品（考虑换行后占用更多空间）
  const maxRowsPerColumn = 45 // 增加到45行（考虑换行后的额外高度）
  ...
  for (const dest of nounyusenGroup) {
    ...
    // 检查商品数量和行数限制
    if (currentItems + itemCount <= maxItemsPerColumn &&
...
function allocateDestinationsToColumns(destinations) {
  ...
  const maxItemsPerColumn = 32 // 调整为32个商品
  const maxRowsPerColumn = 45 // 对应的行数限制为45行
  ...
  while (i < destinations.length) {
    ...
    if (dest.destination_name && dest.destination_name.includes('納入先')) {
      ...
      if (getColumnItemCount(targetColumn) + itemCount <= maxItemsPerColumn &&
        ...
        if (getColumnItemCount(otherColumn) + itemCount <= maxItemsPerColumn &&
...
function getPaginatedData(dateGroup) {
  ...
  const pages = []
  // 调整后的容量：考虑换行后需要更多空间
  const maxItemsPerColumn = 32 // 减少到32个商品
  const maxRowsPerColumn = 45 // 增加到45行
    ...
    while (i < destinations.length) {
      ...
      if (dest.destination_name && dest.destination_name.includes('納入先')) {
        ...
        for (const nounyusenDest of nounyusenGroup) {
          ...
          // 检查商品数量限制
          if (getTempColumnItemCount(currentColumnIndex) + itemCount <= maxItemsPerColumn &&
            ...
            // 当前列容纳不下，切换到另一列
            const otherColumnIndex = 1 - currentColumnIndex
            if (getTempColumnItemCount(otherColumnIndex) + itemCount <= maxItemsPerColumn &&
        ...
        // 严格的页面容量检查
        if (canFitAll &&
            tempColumnSizes[0] <= maxRowsPerColumn && tempColumnSizes[1] <= maxRowsPerColumn &&
            getTempColumnItemCount(0) <= maxItemsPerColumn && getTempColumnItemCount(1) <= maxItemsPerColumn) {
          // 可以放入当前页
          ...
          if (currentPageDestinations.length === 0) {
            ...
            for (const nounyusenDest of nounyusenGroup) {
              ...
              if (getTestColumnItemCount(targetColumn) + itemCount <= maxItemsPerColumn &&
                ...
                const otherColumn = 1 - targetColumn
                if (getTestColumnItemCount(otherColumn) + itemCount <= maxItemsPerColumn &&
...
@page {
  size: A4 portrait;
  margin: 20mm 10mm 6mm 10mm; /* 上边距增加到35mm (约3.5cm) */
}
...
.column {
  flex: 1;
  min-width: 0;
  max-height: 100%; /* 限制列的最大高度 */
  overflow: hidden; /* 防止单列内容溢出 */
}
...
.print-table td {
  ...
  font-size: 15px; /* 字体增大 */
  line-height: 1.2;
  height: auto; /* 改为自动高度，允许内容换行 */
  overflow: visible; /* 显示溢出内容 */
  text-overflow: clip; /* 不使用省略号 */
  white-space: normal; /* 允许换行 */
  word-wrap: break-word; /* 长单词自动换行 */
  word-break: break-all; /* 强制换行，适合长编号 */
}

/* 列宽设置 */
.col-product {
  width: 45%; /* 稍微缩小产品名列，给其他列更多空间 */
}
