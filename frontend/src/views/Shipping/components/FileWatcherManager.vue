...
async function getWatcherStatusData() {
  try {
    ...
    const errorMessage =
      error?.response?.data?.message || error?.message || 'システム状態の取得に失敗しました'
...
async function startWatcher() {
  ...
  try {
    const result = await startFileWatcher()
    ElMessage.success(result?.message || 'ファイル監視器を正常に開始しました')
    await getWatcherStatusData()
  } catch (error: any) {
    console.error('監視器開始エラー:', error)
    const errorMessage =
      error?.response?.data?.message || error?.message || 'ファイル監視器の開始に失敗しました'
    ElMessage.error(errorMessage)
  } finally {
...
async function stopWatcher() {
  ...
  try {
    const result = await stopFileWatcher()
    ElMessage.success(result?.message || 'ファイル監視器を正常に停止しました')
    await getWatcherStatusData()
  } catch (error: any) {
    console.error('監視器停止エラー:', error)
    const errorMessage =
      error?.response?.data?.message || error?.message || 'ファイル監視器の停止に失敗しました'
    ElMessage.error(errorMessage)
  } finally {
...
async function processFile() {
  ...
  try {
    const result = await processFileAPI()
    ...
    const errorMessage =
      error?.response?.data?.message || error?.message || 'ファイル処理の実行に失敗しました'
    ElMessage.error(errorMessage)
  } finally {
...
async function loadStats() {
  try {
    const response = await getShippingLogs({
      page: 1,
      limit: 1,
      statsOnly: true,
    })
    stats.value = {
      ...
      todayRecords: response.todayCount || 0,
      duplicateRecords: response.duplicateCount || 0,
...
async function loadLogs() {
  ...
  try {
    ...
    const response = await getShippingLogs(params)
    logs.value = response.logs || []
    totalRecords.value = response.pagination?.total || 0
  } catch (error: any) {
    console.error('ログ取得エラー:', error)
    const errorMessage =
      error?.response?.data?.message || error?.message || 'ログデータの取得に失敗しました'
    ElMessage.error(errorMessage)
  } finally {
...
async function cleanupLogs() {
  try {
    ...
    const result = await cleanupShippingLogs()
    ...
    if (error !== 'cancel') {
      ...
      const errorMessage =
        error?.response?.data?.message || error?.message || 'ログデータのクリアに失敗しました'
      ElMessage.error(errorMessage)
    }
  }
}
...
async function getDuplicateStats() {
  ...
  try {
    ...
    console.error('重複統計取得エラー:', error)
    const errorMessage =
      error?.response?.data?.message || error?.message || '重複データ統計の取得に失敗しました'
    ElMessage.error(errorMessage)
  } finally {
...
async function performDeduplicate() {
  try {
    ...
    const result = await performDeduplicateAPI()
    ...
    if (error !== 'cancel') {
      ...
      const errorMessage =
        error?.response?.data?.message || error?.message || '重複データの削除に失敗しました'
      ElMessage.error(errorMessage)
    }
  } finally {
...
// 计算重复率
function calculateDuplicateRate(stats: DuplicateStats) {
...
async function loadSyncStatus() {
  try {
    ...
    const errorMessage =
      error?.response?.data?.message || error?.message || '同期状態の取得に失敗しました'
    ElMessage.error(errorMessage)
  }
}
...
async function syncToPickingTasks() {
  try {
    ...
    const result = await syncToPickingTasksAPI()
    ...
    if (error !== 'cancel') {
      ...
      const errorMessage =
        error?.response?.data?.message || error?.message || 'データ同期に失敗しました'
      ElMessage.error(errorMessage)
    }
  } finally {
...
async function createPickingTable() {
  try {
    ...
    const result = await createPickingTableAPI()
    ...
    if (error !== 'cancel') {
      ...
      const errorMessage =
        error?.response?.data?.message || error?.message || 'テーブル作成に失敗しました'
      ElMessage.error(errorMessage)
    }
  }
}
...
async function showSyncDebugInfo() {
  try {
    const response = await getSyncDebugInfoAPI()
    const data = response.data || response

    console.log('同步调试信息:', data)
    ...
    await ElMessageBox.alert(
      ...
      <div style="text-align: left;">
        <h4>データ同期デバッグ情報</h4>
        <p><strong>shipping_log レコード数:</strong> ${data.summary.shippingLogCount}</p>
        <p><strong>picking_tasks レコード数:</strong> ${data.summary.pickingTasksCount}</p>
        <p><strong>マッチしたレコード数:</strong> ${data.summary.matchedCount}</p>
        <p><strong>マッチしなかったレコード数:</strong> ${data.summary.unmatchedCount}</p>
        <br>
    ...
    const errorMessage =
      error?.response?.data?.message || error?.message || 'デバッグ情報の取得に失敗しました'
    ElMessage.error(errorMessage)
  }
}
